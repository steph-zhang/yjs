...
public class PythonJob {
    ...
    public Tuple2<Double, Double> get_salary_tuple(){
        String salary_str = getSalary();
        String[] strs = salary_str.split("-");
        if(strs.length != 2)
            return new Tuple2<>(-1d, -1d);
        if(!(strs[0].endsWith("千") || strs[0].endsWith("万")) ||
                !(strs[1].endsWith("千") || strs[1].endsWith("万")))
            return new Tuple2<>(-1d, -1d);
//        return new Tuple2<>(strs[0], strs[1]);
        return new Tuple2<>(parseSalary(strs[0]), parseSalary(strs[1]));
    }

    private Double parseSalary(String str){
        if(str.endsWith("千")){
            double v = Double.parseDouble(str.substring(0, str.length() - 1));
            return 1000 * v;
